...
if [[ "$INSTALLER_CONTENT" =~ 404\ Not\ Found ]]; then
    echo "ChePanel not supporting this version of distribution"
    echo "Distro: ${DISTRO_NAME} Version: ${DISTRO_VERSION}"
    echo "Exiting..."
    exit 1
fi

# Check is CHE is already installed
if [ -d "/usr/local/che" ]; then
    echo "ChePanel is already installed. Exiting..."
    exit 0
fi

wget $INSTALLER_URL -O ./che-installer.sh
chmod +x ./che-installer.sh
bash ./che-installer.sh
