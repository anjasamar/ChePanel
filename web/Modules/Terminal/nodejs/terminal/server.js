...
const sessionName = 'che_panel_session';
const hostname = execSync('hostname', { silent: true }).toString().trim();

const systemIPs = [];
const terminalConfig = JSON.parse(readFileSync("/usr/local/che/web/storage/app/terminal/config.json").toString());
...
wss.on('connection', (ws, req) => {
    ...
    const file = readFileSync(`/usr/local/che/web/storage/app/terminal/sessions/${sessionID}`);
    if (!file) {
