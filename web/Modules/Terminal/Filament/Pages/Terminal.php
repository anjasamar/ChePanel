...
class Terminal extends Page
{
    ...
    protected function getViewData(): array
    {
        ...
        if (!is_dir('/usr/local/che/web/Modules/Terminal/nodejs/terminal/node_modules')) {
            $exec = shell_exec('cd /usr/local/che/web/Modules/Terminal/nodejs/terminal && npm install');
        }
        if (!is_dir('/usr/local/che/web/storage/logs/terminal')) {
            $exec = shell_exec('mkdir -p /usr/local/che/web/storage/logs/terminal/');
        }
        $exec = shell_exec('node /usr/local/che/web/Modules/Terminal/nodejs/terminal/server.js >> /usr/local/che/web/storage/logs/terminal/server-terminal.log &');
