...
class CustomerResource extends Resource
{
    ...
    {
        $cheServers = [];
        $cheServers[0] = 'Main Server';
        $getCheServers = CheServer::all();
        foreach ($getCheServers as $cheServer) {
            $cheServers[$cheServer->id] = $cheServer->name;
        }

        $schema = [];
        if ($getCheServers->count() > 0) {
            $schema[] = Forms\Components\Select::make('che_server_id')
                ->label('Server')
                ->options($cheServers)
                ->default(0)
                ->hidden(function ($record) {
                    if ($record) {
                        return $record->che_server_id == 0;
                    } else {
                        return false;
                    }
                })
