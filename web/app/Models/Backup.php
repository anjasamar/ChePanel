...
class Backup extends Model
{
    ...
    public function checkCronJob()
    {
        ...
        //$cronJobCommand = 'che-php /usr/local/che/web/artisan che:run-backup-checks';
        ...
        // $cronJobCommand = 'che-php /usr/local/che/web/artisan che:create-daily-full-backup';
        // $findCronJob = CronJob::where('command', $cronJobCommand)->first();
        // if (! $findCronJob) {
        //     $cronJob = new CronJob();
    ...
    public function checkBackup()
    {
        if ($this->status == BackupStatus::Processing) {
            ...
            if (file_exists($backupDoneFile)) {
                ...
                $validateCheConfigFile = $tempValidatePath . '/che-config.ini';

                $errorsBag = [];
                if (! file_exists($validateDatabaseFile)) {
                    $errorsBag[] = 'Database file not found';
                }
                if (! file_exists($validateCheConfigFile)) {
                    $errorsBag[] = 'Che config file not found';
                }
                if (count($errorsBag) > 0) {
                    $this->status = 'failed';
                    $this->backup_log = 'Backup failed. Database or che config file missing.';
                    $this->save();
                    return [
                        'status' => 'failed',
                        'message' => 'Backup failed. Database or che config file missing.',
                        'errors' => $errorsBag
                    ];
                }
                $originalCheConfigContent = file_get_contents(base_path() . '/che-config.ini');
                $backupCheConfigContent = file_get_contents($validateCheConfigFile);
                if ($originalCheConfigContent != $backupCheConfigContent) {
                    $errorsBag[] = 'Che config content mismatch';
                }

                if (count($errorsBag) > 0) {
                    $this->status = 'failed';
                    $this->backup_log = 'Backup failed. Database or che config file content mismatch.';
                    $this->save();
                    return [
                        'status' => 'failed',
    ...
    public function startBackup()
    {
        ...
        $backupFilename = 'che-backup-' . date('Ymd-His') . '.zip';
        $backupFilePath = $storagePath . '/' . $backupFilename;

        // Export Che Panel database
        ...
        $shellFileContent = '';
        $shellFileContent .= 'echo "Backup Che Panel files"' . PHP_EOL;

        // Export Che Panel database
        $mysqlAuthConf = '/root/.che-mysql.cnf';
        $mysqlAuthContent = '[client]' . PHP_EOL;
        $mysqlAuthContent .= 'user="' . CheConfig::get('MYSQL_ROOT_USERNAME') . '"' . PHP_EOL;
        $mysqlAuthContent .= 'password="' . CheConfig::get('MYSQL_ROOT_PASSWORD') . '"' . PHP_EOL;
        file_put_contents($mysqlAuthConf, $mysqlAuthContent);

        $shellFileContent .= 'mysqldump --defaults-extra-file=' . $mysqlAuthConf . ' "' . CheConfig::get('DB_DATABASE') . '" > ' . $databaseBackupPath . PHP_EOL;
        ...
        $backupStructure = [
            'database' => $database,
            'config' => CheConfig::getAll()
        ];
        file_put_contents($backupTempPath . '/backup.json', json_encode($backupStructure, JSON_PRETTY_PRINT));
        $shellFileContent .= 'echo "Backup Che Panel Config"' . PHP_EOL;
        $shellFileContent .= 'cp ' . base_path() . '/che-config.ini ' . $backupTempPath . '/che-config.ini' . PHP_EOL;
        ...
        if ($this->backup_type == 'full') {
            // Export Che Panel Hosting Subscription
        ...
        $shellFileContent .= 'che-php /usr/local/che/web/artisan che:run-backup-checks' . PHP_EOL;
