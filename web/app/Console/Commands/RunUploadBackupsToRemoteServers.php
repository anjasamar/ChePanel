...
class RunUploadBackupsToRemoteServers extends Command
{
     ...
     * @var string
     */
    protected $signature = 'che:run-upload-backups-to-remote-servers';
            ...
            foreach ($getRemoteBackupServers as $remoteBackupServer) {
                ...
                if ($findBackups->count() > 0) {
                    foreach ($findBackups as $backup) {
                        $uploadStatus = $remoteBackupServer->uploadFile($backup->filepath, 'che-system-backups');
                    }
                } else {
                ...
                if ($findHostingSubscriptionBackups->count() > 0) {
                    foreach ($findHostingSubscriptionBackups as $hostingSubscriptionBackup) {
                        $uploadStatus = $remoteBackupServer->uploadFile($hostingSubscriptionBackup->filepath, 'che-hosting-subscription-backups');
                    }
                } else {
