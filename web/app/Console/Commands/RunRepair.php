...
class RunRepair extends Command
{
     ...
     * @var string
     */
    protected $signature = 'che:run-repair';
    ...
    public function handle()
    {
        ...
        // Overwrite supervisor config file
        file_put_contents('/etc/supervisor/conf.d/che.conf', $supervisorConf);
    ...
    public function fixPhpMyAdmin()
    {
        ...
        $ssoContent = file_get_contents('/usr/local/che/web/server/phpmyadmin/che-sso.php.dist');
        if ($ssoContent) {
            file_put_contents('/usr/share/phpmyadmin/che-sso.php', $ssoContent);
        }

        $configContent = file_get_contents('/usr/local/che/web/server/phpmyadmin/config.inc.php.dist');
        if ($configContent) {
            file_put_contents('/usr/share/phpmyadmin/config.inc.php', $configContent);
        }

        $sessionDir = '/usr/local/che/data/sessions';
        ...
        if ($findDatabases->count() > 0) {
            foreach ($findDatabases as $database) {
                ...
                $universalDatabaseExecutor = new UniversalDatabaseExecutor(
                    CheConfig::get('MYSQL_HOST', '127.0.0.1'),
                    CheConfig::get('MYSQL_PORT', 3306),
                    CheConfig::get('MYSQL_ROOT_USERNAME'),
                    CheConfig::get('MYSQL_ROOT_PASSWORD'),
                );
